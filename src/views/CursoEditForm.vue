        ...
        <form class="w-full max-w-lg border-2 p-4 rounded" @submit.prevent="submitForm">
            ...
            <div class="mb-5">
                ...
                <select id="alumnos" v-model="curso.alumnos" multiple class="input-field rounded w-full p-2" required>
                    ...
                    <option v-for="alumno in curso.alumnos" :key="alumno.id" :value="{ alumno }">{{
                        alumno.nombres }}
                    </option>
                </select>
            </div>
...
const arrayToStringAlumnos = (curso_alumnos) => {
    if (!curso_alumnos.alumno) {
        return []
    }
    curso_alumnos.map(e => {
        if (e) {
...
const fetchCurso = async () => {
    try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/cursos/${id}`);
        let data = response.data
        let valor = data
        if (Array.isArray(data.alumnos)) {
            valor = {
                ...data,
                alumnos: arrayToStringAlumnos(data.alumnos)
            };
        }
...
const submitForm = async () => {
    try {
        const response = await axios.post(`${import.meta.env.VITE_API_URL}/cursos/${id}`, {
            ...curso,
